...
- `ping`
- `getYearOverview`: all 12 months
- `getMonthOverview`: one month (1-12)
- `listClassPlans`: optional `from`/`to` (YYYY-MM-DD)
- `getClassPlan`: one plan by date
- `upsertClassPlan`: save one plan, date is the URL not the body
- `batchUpsertClassPlans`: save many at once
- `getNewsletterPrompt`: prompt for an audience + month (1-12)
- `submitMonthlyHabitNewsletters`: submit the package (lifestyle + non-lifestyle required, rest optional)
...
Never write an em-dash, an en-dash used as a dash, or a semicolon in prose. Use a period and a new sentence, a comma, or parentheses. Applies to all class plan text and to newsletter `subject`, `preheader`, `body`.
- Wrong: `isn't passive—it is willingness.` / `Ground down; rise up.`
- Right: `isn't passive. It is willingness.` / `Ground down, then rise up.`
Exceptions: `upas_key_actions` may use semicolons as a list separator, dashes still banned. Numeric ranges use a hyphen (`5-7 breaths`).
